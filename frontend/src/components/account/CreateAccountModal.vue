...
const syncScopedUpstreamModels = async (accountId: number): Promise<any> => {
  if (isUserScope.value) {
    return myResourcesApi.accounts.syncUpstreamModels(accountId)
  }
  return adminAPI.accounts.syncUpstreamModels(accountId)
}
...
const submitCreateAccount = async (payload: CreateAccountRequest) => {
  ...
  try {
    ...
    if (upstreamModelsPreviewed.value || hasConcreteMappedTarget) {
      try {
        const result = await syncScopedUpstreamModels(account.id)
        const warnings = result.warnings ?? []
        if (warnings.some((warning: { code?: string }) => warning.code === 'upstream_model_metadata_incomplete')) {
          appStore.showWarning(t('admin.accounts.syncUpstreamModelsMetadataIncomplete'))
        } else if (warnings.some((warning: { code?: string }) => warning.code === 'upstream_model_metadata_partial')) {
...
const buildOpenAIExtra = (base?: Record<string, unknown>): Record<string, unknown> | undefined => {
  ...
  // OpenAI OAuth-like account writes always converge to one stable fingerprint.
  extra.codex_fingerprint_mode = DEFAULT_CODEX_FINGERPRINT_MODE

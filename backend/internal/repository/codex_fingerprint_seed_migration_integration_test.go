...
func TestMigration231ForcesOpenAIOAuthLikeCodexFingerprintFull(t *testing.T) {
	ctx := context.Background()
	testName := "migration-231-codex-full-" + uuid.NewString()
	migrationSQL, err := dbmigrations.FS.ReadFile("231_force_openai_oauth_codex_fingerprint_full.sql")
	require.NoError(t, err)
	...
	fixtures := []fixture{
		...
		{name: testName + "-setup-token", accountType: service.AccountTypeSetupToken, extra: `{}`},
...
`, f.name, f.accountType, f.extra).Scan(&id))
	...
	require.NoError(t, integrationDB.QueryRowContext(ctx, `SELECT extra->>'codex_fingerprint_mode', extra ? 'codex_fingerprint_seed' FROM accounts WHERE id = $1`, ids[5]).Scan(&apiKeyMode, &apiKeyHasSeed))

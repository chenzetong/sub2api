...
func (s *RedeemCodeRepoSuite) TestHistoryPaginationIsolationAndOrdering() {
	...
	for i := 0; i < 105; i++ {
		...
		code := &service.RedeemCode{
			Code: fmt.Sprintf("HISTORY-%d", i), Type: types[i%len(types)], Status: service.StatusUsed,
			UsedBy: &user.ID, UsedAt: &usedAt,
		}
		s.Require().NoError(s.repo.Create(s.ctx, code))
		ids = append(ids, code.ID)
	}
	otherUsedAt := now.Add(2 * time.Hour)
	s.Require().NoError(s.repo.Create(s.ctx, &service.RedeemCode{
		Code: "OTHER-HISTORY", Type: service.RedeemTypeSubscription, Status: service.StatusUsed,
		UsedBy: &other.ID, UsedAt: &otherUsedAt,
	}))

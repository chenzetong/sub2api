...
// hasStrictOpenAIHeaderSession distinguishes explicit client conversation
// headers from body-derived/cache-derived affinity. Explicit headers promise
// hard account affinity: once claimed, the session must never fail over.
...
func (s *OpenAIGatewayService) selectAccountForModelWithExclusionsStickyHit(ctx context.Context, groupID *int64, platform string, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, stickyAccountID int64, requiredCapability OpenAIEndpointCapability, preferLowUpstreamRate bool) (*Account, bool, error) {
	...
	if IsStrictSessionHash(sessionHash) && stickyAccountID > 0 {
		return nil, false, strictSessionAccountUnavailable(stickyAccountID, "bound OpenAI account is unavailable")
	}
	...
	if sessionHash != "" && (!gatewayProfitControlGateActive(ctx) || IsStrictSessionHash(sessionHash)) {
		if bindErr := s.setStickySessionAccountID(ctx, groupID, sessionHash, selected.ID, openaiStickySessionTTL); bindErr != nil && IsStrictSessionHash(sessionHash) {
	...
	tryAcquireFromLoadMap := func(loadMap map[int64]*AccountLoadInfo) (*AccountSelectionResult, bool, error) {
		...
		for _, item := range selectionOrder {
			...
			if err == nil && result != nil && result.Acquired {
				...
				if sessionHash != "" && !stickySpillover && (!gatewayProfitControlGateActive(ctx) || IsStrictSessionHash(sessionHash)) {

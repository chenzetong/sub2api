...
func (m *XrayRuntimeManager) start(ctx context.Context, proxyID int64, ownerUserID *int64, hash string, outbound map[string]any, blockPrivateDestinations bool) (*xrayRuntimeInstance, error) {
	...
	if err := os.MkdirAll(m.workDir, 0o700); err != nil { //nolint:gosec // workDir is operator configuration, not a request path.
		return nil, err
	}
	...
	if err := os.WriteFile(configPath, rawConfig, 0o600); err != nil { //nolint:gosec // Generated numeric ID and hex digest filename under the operator-controlled workDir.
		return nil, err
	}
	logFile, err := os.OpenFile(logPath, os.O_CREATE|os.O_APPEND|os.O_WRONLY, 0o600) //nolint:gosec // Generated numeric ID and hex digest filename under the operator-controlled workDir.
	if err != nil {
		_ = os.Remove(configPath) //nolint:gosec // Generated numeric ID and hex digest filename under the operator-controlled workDir.
		return nil, err
	}
	...
	if err := cmd.Start(); err != nil {
		_ = logFile.Close()
		_ = os.Remove(configPath) //nolint:gosec // Generated numeric ID and hex digest filename under the operator-controlled workDir.
		_ = os.Remove(logPath)    //nolint:gosec // Generated numeric ID and hex digest filename under the operator-controlled workDir.
		return nil, err
	}
...
func (i *xrayRuntimeInstance) stop() error {
	...
	for _, path := range []string{i.configPath, i.logPath} {
		...
		if err := os.Remove(path); err != nil && !errors.Is(err, os.ErrNotExist) { //nolint:gosec // Instance paths are generated by start under the operator-controlled workDir.
			errs = append(errs, err)
		}
	}
	return errors.Join(errs...)

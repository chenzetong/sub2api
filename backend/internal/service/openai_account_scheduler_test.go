...
func TestOpenAIAdvancedSchedulerStrictSessionRejectsExcludedBoundAccount(t *testing.T) {
	sessionHash := markStrictSessionHash("advanced-bound")
	cacheKey := "openai:" + sessionHash
	cache := &schedulerTestGatewayCache{sessionBindings: map[string]int64{cacheKey: 1}}
	...
	require.Equal(t, int64(1), cache.sessionBindings[cacheKey])
	require.Empty(t, cache.deletedSessions)
}

func TestOpenAIAdvancedSchedulerStrictSessionBindingConflictStopsSelection(t *testing.T) {
	sessionHash := markStrictSessionHash("advanced-first-claim")
	cache := &schedulerTestGatewayCache{
		sessionBindings: map[string]int64{},
		setErr:          ErrStrictSessionAccountConflict,
	}
	svc := &OpenAIGatewayService{
		accountRepo: schedulerTestOpenAIAccountRepo{accounts: []Account{
			{ID: 1, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1},

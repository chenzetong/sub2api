...
func (s *defaultOpenAIAccountScheduler) Select(
	...
	strictSession := IsStrictSessionHash(req.SessionHash)
	if strictSession {
		// Explicit client sessions outrank previous_response/guardian affinity and
		// may never participate in weighted sticky escape. Resolve an existing
		// claim before any layer that could choose a different account.
		req.StickyWeighted = false
		req.GuardianParentAccountID = 0
		req.PreserveStickyBinding = false
		if req.StickyAccountID <= 0 && s != nil && s.service != nil {
		...
		if req.StickyAccountID > 0 {
			...
			if selection == nil || selection.Account == nil {
				return nil, decision, strictSessionAccountUnavailable(req.StickyAccountID, "bound OpenAI account is unavailable")
			}
	...
	if previousResponseID != "" && NormalizeOpenAICompatiblePlatform(req.Platform) == PlatformOpenAI &&
		...
		if selection != nil && selection.Account != nil {
			...
			if req.SessionHash != "" {
				if strictSession {
					if bindErr := s.service.BindStickySession(ctx, req.GroupID, req.SessionHash, selection.Account.ID); bindErr != nil {
						if selection.ReleaseFunc != nil {
							selection.ReleaseFunc()
						}
						return nil, decision, strictSessionBindingError(bindErr)
					}
				} else {
...
func (s *defaultOpenAIAccountScheduler) selectBySessionHash(
	...
	if reason, errorRate, ttft, shouldEscape := s.shouldEscapeStickyAccount(accountID, escapeCfg); shouldEscape && !IsStrictSessionHash(sessionHash) && !req.DisableStickyEscape {
	...
	if s.service.concurrencyService != nil {
		if escapeCfg.enabled && !IsStrictSessionHash(sessionHash) && !req.DisableStickyEscape && acquireErr == nil && result != nil && !result.Acquired {
